...
class LineDetector:
    def __init__(self):
        rospy.loginfo("Line Detector Started!")

        self.sub_cam = rospy.Subscriber("/aero_downward_camera/image", Image, self.image_cb)
        self.pub_param = rospy.Publisher("/error/line", Line, queue_size=1)
    ...
    def segmentLine(self, cv_image):
        ...
        filtered = cv2.inRange(filtered, 230, 255)
        ...
        if DEBUG:
            ...
            if aw.shape[0]:
                ...
                cv2.line(orig, extrapolated, img_center, (0, 255, 0), 1)
                cv2.line(orig, (extrapolated[0], img_center[1]), img_center, (255, 0, 0), 1)
                cv2.line(orig, (img_center[0], extrapolated[1]), img_center, (255, 0, 0), 1)
